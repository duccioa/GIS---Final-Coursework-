...
fetch.lines <- function(sp, id){
    coords <- sp@polygons[id][[1]]@Polygons[[1]]@coords#extract the coordinates of the vertex
    n <- nrow(coords)
    #if the area of the polygon from the coordinates is negative, invert the coordinates
...
Polygon.Cmi_index <- function(sp, id){
    ...
    #write.csv2(V_dataframe, "./debug/V_DataFrame.csv", row.names = FALSE)#DEBUG
    return(C_mi)
}
